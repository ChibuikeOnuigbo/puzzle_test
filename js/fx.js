...
const FX = (() => {
  ...
  const LIGHTS = {
    ...
    conservatory: [
      { src: [985, 84], floorY: 560, spread: [760, 1210], color: "#a8c8da",
        ...
        pool: [[860, 566], [1110, 566], [1090, 596], [884, 596]], op: 0.08, poolOp: 0.1, motes: 10 },
    ],
  };
  ...
  const FLY_ROOMS = { hallway: 0.85, kitchen: 0.9, diningroom: 1.0, attic: 0.7, basement: 0.75, study: 0.5, childroom: 0.4, porch: 0.3, conservatory: 0.55 };
  const FLY_COUNTS = {
    diningroom: [320, 380], kitchen: [110, 140], attic: [90, 120], basement: [80, 110],
    hallway: [60, 90], study: [40, 60], conservatory: [40, 60], childroom: [30, 45], porch: [24, 36],
  };
  ...
  function buildFlyState(room) {
    ...
    const count = Math.round(base[0] + rnd(0, base[1] - base[0]));
    const data = [];
  ...
  function spawnFlies(room) {
    ...
    flies = data.map(f => {
      const el = mk("circle", { class: "fx-fly", cx: f.x, cy: f.y, r: f.r, fill: "#0c0a08", opacity: 0.7 }, group);
